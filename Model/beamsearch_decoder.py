...
if __name__ == '__main__':
    ...
    if args.type == 'test':
        ...
        if args.validation_model == 'best_combined':
            ...
            sample_saver_valid = open(samples_fname + "_valid_" + config['id'] + '_' + str(args.start_epoch) + '.txt',
                                     'w')
            sample_saver_valid = open(samples_fname + "_valid_" + config['id'] + '_' + str(args.start_epoch) + '.txt',
                                     'a')
            checkpoint = torch.load(os.path.join(saved_models, config['id'] + '_' + str(args.start_epoch)))
        Model.load_state_dict(checkpoint['model_State_dict'])
        Model.modelrun(Data=Data_valid, type_='valid', total_step=Data_test.num_batches, ep=0,
