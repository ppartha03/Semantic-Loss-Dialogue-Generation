...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_, hidden, encoder_outputs):
        ...
        attn_weights = F.softmax(
            self.attn(torch.cat((embedded, hidden[0][0]), 1)), dim=1).unsqueeze(0).permute(1,2,0)
...
class EncoderRNN(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        if self.bi_directional:
            out_, hid_ = self.lstm(e.view(-1,1,self.hidden_size), (h0, c0))  # out: tensor of shape (batch_size, seq_length, hidden_size)
            out = out_[:,:,self.hidden_size:] + out_[:,:,:self.hidden_size]
            # Decode the hidden state of the last time step
            return hid_
        else:
            out, hid_ = self.lstm(e.view(-1,1,self.hidden_size), (h0, c0))  # out: tensor of shape (batch_size, seq_length, hidden_size)
            # Decode the hidden state of the last time step
            return out, hid_
