...
parser.add_argument('--run_id', type=int, default=-
                    1)  # run_id of -1 means last run
parser.add_argument('--start_epoch', type=int, default=-1)
parser.add_argument('--num_epochs', type=int, default=100)
parser.add_argument('--seed', type=int, default=100)
# if true, don't mask the words generated after the <eos> token
...
# 'best_meteor' or the model of epoch 'start_epoch'
...
# Create model id
config['id'], config['run_id'] = create_id(
    config, saved_models, args.reload, args.run_id, args.type)
config['wandb_id'] = config['id'] + '_' + \
    str(np.random.randint(10000))  # used for resuming

config['weights'] = np.hstack(
        ...
        for i in range(total_step):
            ...
            if args.type == 'train' and config['output_dropout'] > 0:
                weight_random = np.random.random(
                    len(config['weights']) - 4) > config['output_dropout']
                config['weights'] = np.hstack(
                    [config['weights'][:4], weight_random.astype(int)])
                mask = torch.from_numpy(config['weights']).to(device).bool()
            ...
            if config['prebert_mask']:
                ...
                loss_bert = Bert_loss(
                    self.Bert_embedding(res_postmasked),
                    self.Bert_embedding(tar),
                    config['sentence_embedding'])
...
#                    cnt += 1
        ...
        if type_ == 'eval':
            ...
            logging.info(
                "Valid:   Loss_MLE_eval: {loss_mle_inf:.4f},  Loss_Bert_eval: {loss_bert_inf:.4f}, 'meteor_score': {meteor_score_valid:.2f}\n".format(
                    loss_mle_inf=loss_mle_inf,
                    loss_bert_inf=loss_bert_inf,
                    meteor_score_valid=meteor_score_valid))
        ...
        if type_ == 'train':
            logging.info(
                "Train:   Loss_MLE_train: {loss_mle_inf:.4f},  Loss_Bert_train: {loss_bert_inf:.4f}\n".format(
                    loss_mle_inf=loss_mle_inf,
                    loss_bert_inf=loss_bert_inf))
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mwoz' and (
            args.type == 'train' or args.type == 'valid'):
    ...
    if args.reload and args.type == 'train':
        checkpoint = torch.load(os.path.join(
            saved_models, config['id'] + '_' + str(args.start_epoch)))
        ...
        torch.save({'model_State_dict': Model.state_dict(), 'config': config},
                   os.path.join(saved_models, config['id'] + '_-1'))
    ...
    if args.type == 'train':
        ...
        for epoch in range(config['epoch'], config['num_epochs']):
            ...
            sample_saver_eval = open(
                ...
                '.txt',
                'a')
            loss_mle_valid, combined_loss_valid, meteor_valid = Model.modelrun(
                Data=Data_valid, type_='eval', total_step=Data_valid.num_batches, ep=epoch, sample_saver=sample_saver_eval)
            ...
            if loss_mle_valid < config['best_mle_valid']:
                ...
                # save the best model to wandb
                # torch.save({'model_State_dict': Model.state_dict(), 'config': config},
                # os.path.join(wandb.run.dir, config['id'] +
                # '_best_mle_valid'))
        ...
        if args.validation_model == 'best_combined':
            ...
            sample_saver_valid = open(samples_fname +
                                      "_valid_" +
                                      config['id'] +
                                      '_' +
        ...
        if args.validation_model == 'best_combined':
            ...
            sample_saver_test = open(samples_fname +
                                     "_test_" +
                                     config['id'] +
                                     '_' +
                                     str(args.start_epoch) +
        ...
        Model.modelrun(
            Data=Data_test,
            type_='valid',
            total_step=Data_test.num_batches,
            ep=0,
